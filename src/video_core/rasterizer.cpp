...
// Implemented but not working yet?!
    ...
    auto UpdateStencil = [stencil_test, p, &old_stencil](Pica::Regs::StencilAction action) {
        u8 new_stencil = PerformStencilAction(action, old_stencil, stencil_test.reference_value);
        SetStencil(p.x.Int(), p.y.Int(), (new_stencil & stencil_test.write_mask) | (old_stencil & ~stencil_test.write_mask));
    };

    if (stencil_action_enable) {
        old_stencil = GetStencil(p.x.Int(), p.y.Int());
                                    ...
                                    bool reversed = false)
{
    ...
    // Move rasterizer to sub pixel center
    min_x += half_pixel;
    min_y += half_pixel;
