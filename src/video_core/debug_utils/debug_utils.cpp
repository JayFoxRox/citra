...
static std::string GetTevStageConfigSourceStr(const Pica::Regs::TevStageConfig::Source& source) {
    ...
    auto src_it = source_map.find(source);
...
static std::string GetTevStageConfigColorSourceStr(const Pica::Regs::TevStageConfig::Source& source, const Pica::Regs::TevStageConfig::ColorModifier modifier) {
    ...
    auto src_str = GetTevStageConfigSourceStr(source);
...
static std::string GetTevStageConfigAlphaSourceStr(const Pica::Regs::TevStageConfig::Source& source, const Pica::Regs::TevStageConfig::AlphaModifier modifier) {
...
static std::string GetTevStageConfigOperationStr(const Pica::Regs::TevStageConfig::Operation& operation) {
    ...
    auto op_it = combiner_map.find(operation);
...
std::string GetTevStageConfigColorCombinerStr(const Pica::Regs::TevStageConfig& tev_stage) {
    auto op_str = GetTevStageConfigOperationStr(tev_stage.color_op);
    op_str = ReplacePattern(op_str, "%source1", GetTevStageConfigColorSourceStr(tev_stage.color_source1, tev_stage.color_modifier1));
    op_str = ReplacePattern(op_str, "%source2", GetTevStageConfigColorSourceStr(tev_stage.color_source2, tev_stage.color_modifier2));
    return   ReplacePattern(op_str, "%source3", GetTevStageConfigColorSourceStr(tev_stage.color_source3, tev_stage.color_modifier3));
}

std::string GetTevStageConfigAlphaCombinerStr(const Pica::Regs::TevStageConfig& tev_stage) {
    auto op_str = GetTevStageConfigOperationStr(tev_stage.alpha_op);
    op_str = ReplacePattern(op_str, "%source1", GetTevStageConfigAlphaSourceStr(tev_stage.alpha_source1, tev_stage.alpha_modifier1));
    op_str = ReplacePattern(op_str, "%source2", GetTevStageConfigAlphaSourceStr(tev_stage.alpha_source2, tev_stage.alpha_modifier2));
    return   ReplacePattern(op_str, "%source3", GetTevStageConfigAlphaSourceStr(tev_stage.alpha_source3, tev_stage.alpha_modifier3));
}
...
void DumpTevStageConfig(const std::array<Pica::Regs::TevStageConfig,6>& stages) {
    ...
    for (size_t index = 0; index < stages.size(); ++index) {
        ...
        stage_info += "Stage " + std::to_string(index) + ": " + GetTevStageConfigColorCombinerStr(tev_stage) + "   " + GetTevStageConfigAlphaCombinerStr(tev_stage) + "\n";
    }
